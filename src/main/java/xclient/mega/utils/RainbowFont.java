...
public class RainbowFont extends Font {
    public static RainbowFont WATER = new RainbowFont((s) -> Minecraft.getInstance().fontManager.fontSets.getOrDefault(Minecraft.getInstance().fontManager.renames.getOrDefault(s, s), Minecraft.getInstance().fontManager.missingFontSet)) {
        @Override
        public int drawShadow(PoseStack stack, String text, float x, float y, int color) {
            ...
            for (int i = 0; i < text.length(); i++) {
                int c = color & 0xFF000000 | MathHelper.hsvToRGB(colorr, 0.75F, 1.2F);
                posX = Minecraft.getInstance().font.drawShadow(stack, String.valueOf(text.charAt(i)), posX, y, c);
                colorr += colorrStep;
                colorr %= 1.0F;
            }
            return (int) posX;
